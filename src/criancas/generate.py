...
ONDA = ('<svg class="onda" viewBox="0 0 210 20" preserveAspectRatio="none" aria-hidden="true">'
        '<path d="M0,8 C18,2 34,12 52,7 S88,1 106,7 S142,13 160,7 S196,2 210,7 V20 H0 Z" style="fill:var(--cor)" fill-opacity=".35"/>'
        '<path d="M0,12 C20,7 38,16 58,11 S96,6 116,11 S154,17 174,11 S200,8 210,11 V20 H0 Z" style="fill:var(--cor)"/></svg>')
...
def capa():
    k = art.INK
    g = [f'<path d="M0,300 C80,280 150,300 220,292 C300,282 360,300 420,290 V380 H0 Z" fill="{C["areia"]}" stroke="{k}" stroke-width="3"/>',
         art.planta_caule(30, 300, 190, leaves=10), art.planta_caule(58, 298, 150, color=C['folha3'], leaves=8, flip=-1),
         ...
         f'<g transform="translate(92,78) scale(1.28)">{beto(uid="capa", bubbles=0)}</g>',
         art.bolhas(342, 118, 4),
    ...
    body = (f'<div class="ct"><p class="kicker">Vinte litros de mundo · edição para crianças</p>'
            f'<h1>Meu primeiro<br>aquário</h1><p class="sub">Um livro para montar, esperar, cuidar e brincar</p></div>'
            f'<div class="age">6 a 8<small>anos</small></div>'
            f'<div class="arte">{svg("".join(g), "0 40 420 340", "O Beto, os camarões e as Bacterinhas num aquário plantado")}</div>'
            '<div style="position:absolute; left:24mm; right:24mm; bottom:16mm; z-index:2; background:#fff; border:2.4pt solid var(--ink); border-radius:6mm; padding:4mm 6mm; '
            'font:600 16pt/1 var(--display); color:var(--ink); display:flex; align-items:flex-end; gap:3mm">Este livro é de:<span class="linha" style="flex:1"></span></div>')
...
def meu_livro():
    ...
    items = ''.join(f'<div class="row" style="gap:3mm; align-items:flex-start">{icon(i, "11mm")}<div><b style="font-family:var(--display); font-weight:600">{a}</b><p class="small">{b}</p></div></div>' for i, a, b in legend)
    body = (titulo('Tudo sobre mim') +
            '<p class="lead">Meu nome é <span class="linha" style="min-width:140mm"></span></p>'
            '<div class="g2" style="grid-template-columns:1fr 78mm; align-items:stretch">'
            ...
            '<p class="lead">Quem me ajuda é <span class="linha" style="min-width:60mm"></span>.</p>'
            f'{fala(BETO(), "<p>Oi! Eu sou o <b>Beto</b>. Vou te ensinar tudo sobre a minha casa nova.</p>", "40mm")}'
            '</div><div class="desenho" style="height:74mm">Desenhe você aqui</div></div>'
            f'<h3 class="h">Os sinais deste livro</h3><div class="g2" style="gap:4mm 6mm">{items}</div>'
...
def combinado():
    body = (titulo('O combinado', 'Cuidar de bichos é coisa séria — e muito divertida. Antes de começar, vamos fazer um combinado?') +
            '<div class="box jogo"><p class="h">Eu, <span class="linha" style="min-width:90mm"></span>, prometo:</p>'
            ...
            '<p style="margin-top:5mm">Assinatura: <span class="linha" style="min-width:110mm"></span></p></div>'
            '<div class="box adulto"><p class="h">E eu, adulto, <span class="linha" style="min-width:80mm"></span>, prometo:</p>'
            '<ul class="bol check" style="--cor:#fff">'
            ...
            '<p style="margin-top:5mm">Assinatura: <span class="linha" style="min-width:110mm"></span></p></div>'
            '<p class="lead center">Data do combinado: <span class="linha" style="min-width:16mm"></span> / <span class="linha" style="min-width:16mm"></span> / <span class="linha" style="min-width:24mm"></span></p>')
    return page('combinado', 'coral', body)


def turma():
    cards = [
        (BETO(), 'Beto, o betta', 'Um peixe colorido e curioso que respira ar. Vai ser o rei da superfície. (O seu pode ter outro nome!)'),
        (CEREJA(), 'Cereja e as irmãs', 'Camarõezinhos vermelhos que passam o dia limpando o aquário. Vão morar em turma.'),
    ...
    cs = ''.join(f'<div class="card"><div class="pic">{s}</div><b>{n}</b><p>{t}</p></div>' for s, n, t in cards)
    body = titulo('Conheça a turma', 'Estes são os moradores do aquário — e quem cuida deles.') + f'<div class="g3">{cs}</div>' + \
...
def mundo():
    body = (titulo('Um aquário é um mundo pequeno', 'Dentro do vidro, cada morador ajuda o outro. É uma roda que não para de girar.') +
            f'<div style="width:150mm; margin:0 auto">{art.ciclo()}</div>' +
...
def trilha():
    ...
    tlh = ''.join(f'<div style="flex:1; text-align:center"><div style="height:10mm; border:2pt solid var(--ink); border-radius:5mm; background:var(--{c}); font:600 13pt/9.6mm var(--display)">{d}</div><p class="small" style="margin-top:1.4mm">{t}</p></div>' for d, t, c in tl)
    body = (titulo('A trilha do aquarista', 'O aquário se monta em 9 etapas, uma depois da outra. Siga a trilha!') +
            f'<div style="width:168mm; margin:0 auto">{art.trilha(stops)}</div>'
            '<h3 class="h">Quanto tempo leva?</h3>'
            f'<div class="row" style="gap:3mm; align-items:flex-start">{tlh}</div>'
...
def regras():
    ...
    items = ''.join(f'<li><div class="row" style="gap:3mm">{icon(i, "10mm")}<p>{t}</p></div></li>' for i, t in rs)
    body = (titulo('Regras de ouro', 'Oito regras para um aquário feliz e um aquarista seguro. Leia com um adulto.') +
            f'<ol class="num" style="--cor:var(--sol)">{items}</ol>' +
...
def e1_caixa():
    ...
    cs = ''.join(f'<div class="card"><div class="pic" style="height:22mm">{art.aparelho(a)}</div><b>{n}</b><p>{t}</p></div>' for a, n, t in it)
    body = titulo('A caixa de ferramentas', 'Oito coisas fazem o aquário funcionar. Vamos conhecer cada uma?') + f'<div class="g4">{cs}</div>' + \
        box('adulto', 'Com um adulto', '<p>O filtro, o aquecedor e a luz vão na tomada. <b>Quem liga, desliga e mexe nos fios é sempre o adulto.</b></p>')
...
def e1_ligue():
    left = ''.join(f'<div class="row" style="height:19mm; justify-content:space-between"><div style="width:19mm">{art.aparelho(a)}</div><span class="circ"></span></div>' for a, _ in LIGUE)
    right = ''.join(f'<div class="row" style="height:19mm"><span class="circ"></span><p style="font-size:14pt">{LIGUE[i][1]}</p></div>' for i in LIGUE_DIR)
    litros = ''.join('<span style="display:inline-block; width:6.6mm; height:11mm; margin-right:1.3mm; border:1.6pt solid var(--ink); border-radius:1.6mm 1.6mm 2.4mm 2.4mm; background:#fff"></span>' for _ in range(20))
    body = (titulo('Jogo: ligue cada coisa ao que ela faz', 'Faça uma linha do círculo da esquerda até o círculo da direita.') +
            f'<div class="box jogo" style="display:grid; grid-template-columns:44mm 1fr; gap:16mm">'
            f'<div>{left}</div><div>{right}</div></div>'
            '<h3 class="h">As contas do aquário</h3>'
            ...
            f'<div class="box jogo"><p class="bh">{icon("lapis", "8mm")}Cada potinho é 1 litro. Pinte de azul os 13 litros de água.</p><div style="white-space:nowrap">{litros}</div></div>')
...
def e2_lugar():
    ...
    cs = ''.join(f'<div class="card"><div class="row" style="justify-content:space-between"><b style="font-size:20pt">{l}</b><span class="circ" style="width:11mm; height:11mm"></span></div>'
                 f'<div style="height:44mm; display:flex; justify-content:center">{alto(k)}</div><p>{t}</p></div>' for l, k, t in ps)
    body = (titulo('Onde o aquário vai morar?', 'Qual destes lugares é o melhor? Pinte o círculo do lugar certo.') +
            f'<div class="g2">{cs}</div>'
            '<div class="g2">' +
            box('dica', 'Limpar sem sabão', '<p>O vidro se lava <b>só com água</b>. Sabão, detergente e álcool deixam restinhos que fazem mal aos bichos.</p>') +
            box('exp', 'O teste do vazamento', '<p>O adulto enche o aquário e vocês esperam <b>um dia inteiro</b>. Depois, passem papel nos cantinhos do vidro. Está sequinho? Oba, não vaza!</p>') +
            '</div>')
...
def e3_chao():
    body = (titulo('O chão é um bolo de camadas', 'Embaixo vai a terra fértil: é o recheio que alimenta as raízes. Em cima vai a areia: é a cobertura.') +
            f'<div style="width:150mm; margin:0 auto">{art.camadas()}</div>'
            ...
            '<ol class="num" style="--cor:var(--sol)">'
            ...
            '<div class="g2">' +
            box('sabia', 'Você sabia?', '<p>Cada grão de areia é a casa de muitas Bacterinhas. O chão também ajuda a limpar a água!</p>') +
            box('jogo', 'Conta rápida', '<p>Vamos usar <b>5 quilos</b> de areia: é o peso de 5 pacotes de açúcar de 1 quilo.</p>') + '</div>')
...
def e4_paisagem():
    ...
    cs = ''.join(f'<div><div style="position:relative">{mini(m)}<div style="position:absolute; right:-2mm; top:-3mm">{icon(i, "10mm")}</div></div><p class="small" style="margin-top:1.5mm">{t}</p></div>' for m, i, t in tips)
    body = (titulo('Pedras e troncos: a paisagem', 'Antes das plantas, montamos as pedras e o tronco. Eles são os ossos da paisagem.') +
            f'<div class="g3">{cs}</div>'
            '<div class="g2">' +
            box('dica', 'Dica do Beto', '<p>Deixe um espaço livre na frente: é a minha <b>piscina</b>! E faça frestinhas e cavernas: são o esconderijo dos camarões.</p>') +
            box('adulto', 'Com um adulto', '<p>O tronco é fervido na panela antes de ir para o aquário. Isso limpa e ajuda ele a afundar. Panela quente é trabalho do adulto.</p>') +
            '</div>'
            '<h3 class="h">Desenhe a sua paisagem</h3>'
            f'<div class="grow" style="display:flex; align-items:center; justify-content:center"><div style="width:160mm">{svg(art.aquario(400, 220, water=False), "-4 -4 408 228", "Aquário vazio para desenhar")}</div></div>')
...
def exp_pedra():
    ...
    body = (titulo('A pedra que faz bolhinhas', 'Algumas pedras mudam a água do aquário e deixam ela “dura”. Vamos descobrir quais, como cientistas de verdade!') +
            f'<div class="g2" style="grid-template-columns:1fr 70mm; align-items:center"><div>'
            ...
            '<table class="tab" style="--cor:var(--folha2)"><tr><th>Pedra</th><th>Fez bolhinhas?</th><th>Aprovada?</th></tr>'
            ...
            '</table><div class="g2">' +
            box('sabia', 'Por que borbulha?', '<p>O vinagre é um <b>ácido</b>. Quando encontra o <b>calcário</b> da pedra, os dois reagem e soltam um gás: as bolhinhas!</p>') +
            box('exp', 'Teste da meia-calça', '<p>Passe uma meia-calça velha na pedra e no tronco. Enroscou? Tem uma ponta que pode rasgar a nadadeira do Beto.</p>') + '</div>')
...
def e5_plantas():
    ...
    body = (titulo('As plantas trabalham muito', 'Elas parecem só enfeite, mas têm quatro superpoderes.') +
            f'<div class="g2" style="grid-template-columns:1fr 1fr; gap:4mm">{cs}</div>' +
            '<div class="g2" style="grid-template-columns:1fr 72mm; align-items:start"><div>'
            ...
            '<li>As plantas que <b>boiam</b> são as últimas: vão depois que a água já está lá.</li></ul></div>'
            + foto('plant-aquario.jpg', 60, cap='Um aquário bem plantado.') + '</div>' +
            box('dica', 'Dica do Beto', '<p>Plante <b>bastante</b> desde o primeiro dia. Assim sobra pouco espaço para as algas, aquelas plantinhas verdes que sujam o vidro.</p>'))
...
def cartas(n):
    ...
    body = (titulo(f'Cartas das plantas · {n} de 2', 'Cada planta tem um superpoder. Qual é a sua preferida? Pinte o coração dela!' if n == 1 else None) +
            '<div class="g2 grow" style="grid-template-rows:1fr 1fr">' + ''.join(carta(c) for c in grupo) + '</div>')
...
def borda():
    ...
    cs = ''.join(f'<div>{foto(i, 34)}<p style="margin-top:.5mm"><b style="font-family:var(--display); font-weight:600; font-size:14pt">{n}</b></p><p class="small">{t}</p></div>' for i, n, t in ps)
    dias = ''.join(f'<td style="height:24mm; border:1.8pt solid var(--ink); border-radius:3mm; vertical-align:bottom; text-align:center; font:600 11pt/1 var(--display); padding-bottom:1.5mm">dia {d}</td>' for d in (1, 3, 5, 7, 10, 14))
    body = (titulo('Plantas que moram na borda', 'Algumas plantas de casa ficam com as folhas para fora e só as raízes dentro da água. As raízes bebem a sujeira!') +
            f'<div class="g3">{cs}</div>' +
            box('exp', 'Experiência: a muda que cria raiz',
                ...
                f'<table style="width:100%; border-collapse:separate; border-spacing:1.6mm"><tr>{dias}</tr></table>'
                '<p class="small">Em umas 2 semanas, a muda está pronta para ir para a borda do aquário!</p>') +
            box('adulto', 'Atenção', '<p><b>Nunca coma nenhuma planta.</b> A jiboia faz mal se for mastigada — para pessoas, gatos e cachorros.</p>', 'atencao'))
...
def mapa_pg():
    ...
    body = (titulo('O mapa do nosso aquário', 'Assim é o aquário visto de cima, como um passarinho veria. Pinte cada lugar com a cor da planta!') +
            f'<div style="width:176mm; margin:0 auto">{art.mapa()}</div>'
            f'<div class="g4" style="gap:4mm">{ls}</div>'
            '<p class="lead">O <b>tronco</b> é marrom e as <b>pedras</b> são cinza. O espaço branco da frente é a <b>piscina do Beto</b>: lá não vai planta nenhuma!</p>'
            + fala(CEREJA(), '<p>O musgo (6) fica grudado no tronco. É lá que os meus filhotes vão morar!</p>', '36mm'))
...
def quadro(inner, legenda, n):
    return (f'<div class="card" style="padding:2.4mm"><div style="position:relative">{svg(inner, "0 0 200 130")}'
            ...
            f'<p style="margin-top:2mm; font-size:12.5pt; line-height:17pt">{legenda}</p></div>')
...
def e6_bacterinhas():
    ...
    body = (titulo('As Bacterinhas: heroínas invisíveis', 'Antes de os bichos chegarem, o aquário precisa de um time de limpeza. Ele é tão pequeno que só dá para ver no microscópio!') +
            '<div class="g2">' + ''.join(quadro(q, l, i + 1) for i, (q, l) in enumerate(qs)) + '</div>' +
            box('sabia', 'Você sabia?', '<p>As Bacterinhas moram grudadas na <b>esponja do filtro</b>, nas pedras e na areia. Por isso a esponja nunca é lavada na torneira: o cloro da água mataria o time todo!</p>'))
    return page('e6-bacterinhas', 'roxo', body, 'Etapa 6 · A espera', 'sabia')


def e6_detetive():
    cores = [('#F4E27A', '0'), ('#D8E27A', '0,5'), ('#9CCB6E', '1'), ('#4E9A58', '4')]
    tubos = ''.join(f'<div style="width:16mm">{art.tubo(c, l)}</div>' for c, l in cores)
    vazios = ''.join(f'<div style="text-align:center"><div style="width:15mm; margin:0 auto">{art.tubo("#fff", "")}</div><p class="small">dia <span class="linha" style="min-width:10mm"></span>/<span class="linha" style="min-width:10mm"></span></p></div>' for _ in range(4))
    body = (titulo('Detetive da água', 'Como saber se as Bacterinhas já chegaram? Com testes! O adulto pinga o líquido no tubinho. Você compara as cores.') +
            '<div class="box jogo"><p class="bh">Exemplo: o teste de amônia</p>'
            ...
            '<span style="width:30mm">ótimo!</span><span style="width:30mm">atenção</span><span style="width:30mm">perigo</span><span style="width:30mm">muito perigo</span></div>'
            ...
            '<h3 class="h">Pinte o tubo com a cor que apareceu</h3>'
            f'<div class="g4">{vazios}</div>' +
            '<div class="g2">' +
            box('sabia', 'Por que esperar tanto?', '<p>As Bacterinhas levam umas <b>6 semanas</b> para crescer. Se os bichos chegassem antes, a amônia faria mal para eles. <b>Esperar é cuidar!</b></p>') +
            box('adulto', 'Só o adulto', '<p>O líquido do teste é <b>venenoso</b>. Quem pinga e guarda os frascos é o adulto. Você observa e compara.</p>') + '</div>')
...
def contagem():
    ...
    body = (titulo('Contagem regressiva', 'Pinte uma bolinha por dia. Quando chegar na vermelha, os camarões podem vir. Na azul, é a vez do Beto!') +
            f'<table class="cal"><tr>{head}</tr>{"".join(rows)}</table>'
            '<div class="leg"><span><i style="background:var(--sol)"></i>1º de janeiro: montar o aquário</span><span><i style="background:var(--roxo2)"></i>5 de fevereiro: grande teste</span>'
    ...
    return page('contagem', 'roxo', body, 'Etapa 6 · A espera', 'lapis')
...
def e7_cerejinhas():
    ...
    fs = ''.join(f'<div class="card" style="background:var(--coral2)"><b>{a}</b><p>{b}</p></div>' for a, b in fatos)
    cores = [('neo-cereja.jpg', 'Cereja'), ('neo-sakura.jpg', 'Sakura'), ('neo-verde.jpg', 'Verde')]
    cs = ''.join(f'<div>{foto(i, 21)}<p class="small center" style="margin-top:-.5mm"><b>{n}</b></p></div>' for i, n in cores)
    body = (titulo('As Cerejinhas', 'Os camarões chegam antes do Beto. Eles são pequenos, mas fazem um trabalhão!') +
            f'<div class="g3" style="gap:4mm">{fs}{"".join([])}</div>'
            '<h3 class="h">Existem camarões de muitas cores</h3>'
            f'<div class="g3">{cs}</div>'
            '<div style="display:grid; grid-template-columns:auto 1fr; gap:5mm; align-items:stretch">'
            '<div class="box jogo"><p class="bh">' + icon('lapis', '8mm') + 'Tamanho de verdade</p>' + art.regua('cereja') +
            '<p class="small" style="max-width:90mm">A Cereja adulta tem 3 cm. O filhote é do tamanho do pontinho vermelho!</p></div>' +
            box('adulto', 'Muito importante', '<p><b>Nunca solte</b> camarões, peixes ou plantas em rios, lagos ou ralos. Eles atrapalham os bichos que já moram lá. Se sobrar, dê para outro aquarista.</p>', 'atencao') +
            '</div>')
...
def e7_chegada():
    ...
    body = (titulo('A chegada dos camarões', 'Eles chegam num saquinho com a água da loja. A água nova é diferente: eles precisam de tempo para se acostumar.') +
            f'<div class="g2" style="grid-template-columns:1fr 62mm; align-items:start">'
            ...
            '<ul class="bol" style="--cor:var(--roxo2)"><li>Nos primeiros dias, os camarões ficam escondidos.</li>'
            '<li>Aparece uma casca vazia: alguém trocou de roupa!</li>'
            '<li>De repente, todos nadam agitados: é época de namoro.</li></ul></div>' +
            box('dica', 'Quantos e quais?', '<p>Vêm de <b>15 a 20</b> camarões, todos da mesma cor. Se misturar cores, os filhotes nascem marronzinhos, como os camarões da natureza.</p>'))
...
def jogo_camaroes():
    ...
    body = (titulo('Jogo: onde estão os camarões?', 'Sete camarões se esconderam entre as plantas. Encontre todos e circule cada um!') +
            f'<div class="box jogo">{art.cena_escondidos()}</div>'
            f'<p class="lead">A cada camarão que achar, pinte uma bolinha:</p><div class="row" style="justify-content:center">{bol}</div>' +
            fala(CEREJA(), '<p>No aquário de verdade, a gente também se esconde. Procure à noite com uma lanterna: é a hora em que mais aparecemos!</p>', '38mm'))
    return page('jogo-camaroes', 'coral', body, 'Jogo', 'jogo')
...
def e8_beto():
    ...
    fs = ''.join(f'<div class="card" style="background:var(--agua2)"><b>{a}</b><p>{b}</p></div>' for a, b in fatos)
    body = (titulo('Beto, o peixe que respira ar') +
            '<div class="g2" style="grid-template-columns:1fr 1fr; align-items:stretch">' + foto('betta-halfmoon-azul.jpg', 70, '50% 25%', 'Um betta de verdade, com as nadadeiras abertas.') +
            f'<div style="display:flex; flex-direction:column; justify-content:center; gap:3mm">{fala(BETO(), "<p>Muito prazer! Agora é a minha vez de contar quem eu sou.</p>", "34mm")}'
            + box('sabia', 'Meu nome científico', '<p><i>Betta splendens</i>. <i>Splendens</i> quer dizer <b>brilhante</b>!</p>') + '</div></div>'
            f'<div class="g3" style="gap:4mm">{fs}</div>'
            '<div class="box jogo" style="display:flex; gap:6mm; align-items:center">' + art.regua('beto') +
...
def e8_caudas():
    fotos = [('betta/cauda-veu.jpg', 'Véu', 'Cauda comprida, caída como um véu.'), ('betta/halfmoon.jpg', 'Meia-lua', 'Abre a cauda como uma lua pela metade.'),
             ...
             ('betta/wild-splendens.jpg', 'Da natureza', 'Assim é o betta nos arrozais da Tailândia.')]
    cs = ''.join(f'<div>{foto(i, 34)}<div class="row" style="justify-content:space-between; margin-top:-.5mm"><b style="font:600 13pt/1.1 var(--display)">{n}</b>{icon("coracao", "7mm")}</div><p class="small">{t}</p></div>' for i, n, t in fotos)
    body = (titulo('Caudas e cores', 'Existem bettas de muitos jeitos. Pinte o coração do seu preferido!') + f'<div class="g3" style="gap:4mm 5mm">{cs}</div>')
...
def e8_sente():
    ...
    rows = ''.join(f'<tr><td style="width:46mm; padding:1mm 3mm">{BETO(uid="s" + str(i), **kw)}</td><td><b style="font:600 15pt/1.2 var(--display)">{n}</b><p class="small">{d}</p></td><td style="width:62mm"><p class="small"><b>{o}</b></p></td></tr>'
    ...
    body = (titulo('O que o Beto está sentindo?', 'O Beto não fala, mas mostra tudo com o corpo. Aprenda a ler os sinais!') +
            f'<table class="tab" style="--cor:var(--agua2)"><tr><th>Como ele fica</th><th>O que é</th><th>O que fazer</th></tr>{rows}</table>' +
            '<div class="g2">' +
            box('sabia', 'E o ninho de bolhas?', '<p>Um montinho de bolhas grudadas na superfície quer dizer que ele está <b>contente</b> e se sentindo em casa.</p>') +
            box('jogo', 'Brincadeira de mímica', '<p>Um faz a cara e o corpo do Beto — feliz, se exibindo, com medo, dormindo — e o outro tenta adivinhar!</p>') +
            '</div>')
    return page('e8-sente', 'agua', body, 'Etapa 8 · O Beto', 'coracao')


def e8_chegou():
    body = (titulo('O Beto chegou!', 'Ele vem depois dos camarões, entre <b>12 de março e 9 de abril</b>.') +
            '<p class="lead">O nome do meu betta vai ser: <span class="linha" style="min-width:80mm"></span></p>'
            ...
            '<div class="desenho grow" style="min-height:70mm">Desenhe o seu betta, com as cores dele</div>'
            '<h3 class="h">Os primeiros dias</h3>'
            '<ol class="num" style="--cor:var(--agua)">'
            ...
            fala(BETO(), '<p>Eu reconheço quem me dá comida! Em poucos dias, vou nadar até a frente do vidro quando você chegar.</p>'))
    return page('e8-chegou', 'agua', body, 'Etapa 8 · O Beto', 'lapis')


def e9_tarefas():
    eu, ad, jt = icon('eu', '9mm'), icon('adulto', '9mm'), icon('adulto', '9mm')
    ...
    rows = ''.join(f'<tr><td style="width:46mm"><b>{q}</b></td><td>{t}</td><td class="q">{w}</td></tr>' for q, t, w in ts)
    body = (titulo('Minhas tarefas de aquarista', 'Um pouquinho todo dia vale mais que muito de vez em quando.') +
            f'<table class="tab" style="--cor:var(--sol2)"><tr><th>Quando</th><th>O que fazer</th><th class="q">Quem</th></tr>{rows}</table>'
            ...
            '<div class="g2">' +
            box('dica', 'A regra do olhinho', '<p>A barriga do Beto é do tamanho do <b>olho</b> dele. Comida demais faz mal e suja a água.</p>') +
            box('adulto', 'Dica para o adulto', '<p>Separe as porções da semana num potinho com divisões. Assim a criança alimenta sozinha, sem errar a medida.</p>') +
            '</div><p class="lead">Use a tabela de estrelas da página {{pg:estrelas}} para marcar tudo!</p>')
...
def termometro_pg():
    cs = ''.join(f'<div class="card center"><div style="width:40mm; margin:0 auto">{art.termometro(t)}</div>'
                 f'<div class="row" style="justify-content:center; gap:6mm; margin-top:2mm"><span class="row" style="gap:1.5mm">{art.smile("feliz")}<span class="small">tudo bem</span></span>'
                 f'<span class="row" style="gap:1.5mm">{art.smile("triste")}<span class="small">chamar adulto</span></span></div></div>' for t in TERMOS)
    reta = ''.join(f'<div style="flex:1; text-align:center; border-left:1.5pt solid var(--ink); padding-top:1mm; font:600 11pt/1 var(--display); background:{"var(--folha2)" if 24 <= n <= 26 else "transparent"}">{n}</div>' for n in range(20, 33))
    body = (titulo('Aprenda a ler o termômetro', 'O número bom para o aquário é <b>24, 25 ou 26</b>. Olhe cada termômetro e pinte a carinha certa.') +
            ...
            '<h3 class="h">A régua da temperatura</h3>'
            ...
            box('adulto', 'No verão', '<p>Em janeiro, a água pode esquentar demais. <b>Se passar de 28, chame um adulto.</b> E nunca jogue gelo dentro do aquário!</p>', 'atencao'))
...
def sos_pg():
    ...
    cs = ''.join(f'<div class="card center" style="padding:2mm"><div style="width:32mm; margin:0 auto">{art.sos(k)}</div><p class="small" style="margin-top:1mm">{t}</p></div>' for k, t in ss)
    body = (titulo('Chame um adulto!', 'Se você vir alguma destas coisas, avise um adulto na hora. Você é o vigia do aquário!') +
            f'<div class="g4" style="gap:3.5mm">{cs}</div>'
            '<div class="box sabia"><p class="bh">' + icon('sabia', '8mm') + 'Isso é normal, não se assuste</p>'
            '<ul class="bol" style="--cor:var(--roxo2)"><li>Uma casca vazia de camarão: ele trocou de roupa.</li>'
...
def cacapalavras():
    ...
    body = (titulo('Caça-palavras', 'Encontre 10 palavras do aquário. Elas estão deitadas ou em pé. Marque cada uma que achar!') +
            f'<div class="box jogo">{art.grade_letras(GRID, cell=13.2)}</div>'
            f'<ul class="bol check" style="columns:5; font:600 13pt/1.2 var(--display); --cor:#fff">{lista}</ul>'
            '<p class="small">No caça-palavras não tem acento: CAMARÃO vira CAMARAO e ÁGUA vira AGUA.</p>')
    return page('cacapalavras', 'agua', body, 'Jogos', 'jogo')


def labirinto_pg():
    body = (titulo('Labirinto', 'Ajude a Cereja a chegar ao musgo, onde ela fica protegida.') +
            f'<div class="box jogo grow" style="display:flex; justify-content:center"><div style="width:150mm">{art.labirinto(11, 13, seed=11, cell=30)}</div></div>')
    return page('labirinto', 'folha', body, 'Jogos', 'jogo')
...
def diferencas():
    body = (titulo('Jogo das 5 diferenças', 'Os dois aquários parecem iguais, mas têm 5 diferenças. Circule todas no desenho de baixo!') +
            f'<div class="box jogo" style="width:138mm; margin:0 auto">{art.cena()}</div><div class="box jogo" style="width:138mm; margin:0 auto">{art.cena(DIFFS)}</div>'
...
def colorir1():
    body = (titulo('Pinte o aquário', 'Use muitas cores! O Beto pode ser azul, vermelho, amarelo... do jeito que você quiser.') +
            f'<div class="grow" style="display:flex; align-items:center"><div style="width:100%">{art.cena(line=True)}</div></div>')
    return page('colorir1', 'sol', body, 'Jogos', 'lapis')


def colorir2():
    body = (titulo('Pinte a Cereja e as Bacterinhas') +
            f'<div style="width:150mm; margin:0 auto">{art.cereja_svg(line=True)}</div>'
            f'<div style="width:170mm; margin:0 auto">{svg(bacterinha(line=True) + bacterinha(line=True, x=100, y=10, mood="comendo"), "-12 6 196 60")}</div>'
            f'<div style="width:110mm; margin:0 auto">{art.beto_svg(line=True, mood="dormindo")}</div>')
...
def quiz():
    ...
    body = (titulo('Verdadeiro ou falso?', 'Pinte o V se for verdade e o F se for mentira. Você já sabe tudo isso!') +
            f'<div class="box jogo"><div class="vf" style="font-size:14.5pt; line-height:19pt">{rows}</div></div>' +
            fala(BETO(mood='feliz'), '<p>Acertou quase tudo? Então você já é um aquarista de verdade! As respostas estão na página {{pg:respostas}}.</p>'))
    return page('quiz', 'roxo', body, 'Jogos', 'jogo')


def diario(n):
    entrada = ('<div class="card" style="display:flex; flex-direction:column; gap:3mm; flex:1">'
               '<div class="row" style="justify-content:space-between"><p class="lead">Dia <span class="linha" style="min-width:12mm"></span> / <span class="linha" style="min-width:12mm"></span> / 2027</p>'
               f'<div class="row" style="gap:2mm"><span class="small">O Beto estava:</span>{art.smile("feliz")}{art.smile("meio")}{art.smile("triste")}</div></div>'
               '<div class="g2" style="grid-template-columns:78mm 1fr; flex:1"><div class="desenho">Hoje eu vi...</div><div class="linhas"></div></div></div>')
    body = titulo('Meu diário de aquarista', 'Desenhe e escreva o que você viu no aquário.' if n == 1 else None) + \
        f'<div class="grow" style="display:flex; flex-direction:column; gap:5mm">{entrada}{entrada}</div>'
...
def dicionario():
    ...
    body = titulo('Dicionário do aquarista', 'Palavras novas que você aprendeu neste livro.') + f'<div style="columns:2; column-gap:8mm">{items}</div>'
...
def respostas():
    ...
    mini_grade = art.grade_letras(GRID, PLACED, cell=6.2).replace('class="cp"', 'class="cp mini"')
    ...
    body = (titulo('Respostas', 'Só olhe depois de tentar!') +
            ...
            f'<div><h3 class="h">Caça-palavras</h3>{mini_grade}</div>'
            f'<div><h3 class="h">Labirinto</h3><div style="width:62mm">{art.labirinto(11, 13, seed=11, cell=30, solution=True)}</div></div></div>'
            '<div class="g2 small" style="align-items:start; gap:3mm 8mm">'
            ...
            f'<div style="grid-column:1 / -1"><h3 class="h">Ligue cada coisa</h3><p>{ligue_r}.</p></div></div>')
...
def adultos():
    body = (titulo('Para os adultos', 'Este livro acompanha o guia <i>Vinte litros de mundo</i>: as mesmas etapas, datas e números, numa linguagem para crianças de 6 a 8 anos.') +
            '<div class="g2" style="align-items:start; font-size:11.5pt; line-height:15.5pt">'
            ...
            '<h3 class="h" style="margin-top:4mm">O que a criança pode fazer</h3><p>Observar, contar, ler o termômetro, alimentar com a porção já separada, lavar a areia, limpar o vidro por fora, anotar no diário e avisar quando algo parecer errado.</p>'
            ...
            '<h3 class="h" style="margin-top:4mm">Responsabilidade</h3><p>A criança ajuda, mas o bem-estar dos animais é do adulto: férias, esquecimentos e doenças acontecem. Um camarão pode morrer, e o betta vive de 2 a 4 anos. Falar disso com honestidade faz parte do aprendizado.</p></div></div>'
            '<div class="box dica" style="font-size:11.5pt; line-height:15.5pt"><p class="bh">' + icon('dica', '8mm') + 'Para imprimir</p>'
            '<p>A tabela de estrelas (página {{pg:estrelas}}) está em A4 paisagem: imprima sozinha, em papel grosso, e plastifique. As respostas dos jogos estão na página {{pg:respostas}}. Os detalhes técnicos de cada etapa — quantidades, parâmetros da água, doenças — estão no guia para adultos e no <i>Diário 2027</i>.</p></div>')
...
def creditos():
    body = (titulo('Créditos das fotos', 'As fotos deste livro são de uso livre, com as licenças indicadas. Licenças NC permitem apenas uso não comercial.') +
            '<div class="credits" style="columns:2; column-gap:8mm; font-size:9pt; line-height:12pt"><!--#credits--></div>'
            '<p class="small">Desenhos feitos para este livro. Fontes: Fredoka e Atkinson Hyperlegible Next, ambas com licença livre (SIL Open Font License). '
...
def certificado():
    ...
    body = ('<div style="flex:1; border:3pt solid var(--ink); border-radius:8mm; outline:2pt dashed var(--agua); outline-offset:-5mm; padding:14mm 12mm; display:flex; flex-direction:column; align-items:center; gap:6mm; text-align:center; background:#fff">'
            f'<div style="width:120mm">{deco}</div>'
            '<p style="font:700 34pt/1 var(--display); color:var(--ink)">Certificado de<br>Pequeno Aquarista</p>'
            '<p class="lead">Certificamos que</p><span class="linha" style="width:140mm; height:12mm"></span>'
            '<p class="lead" style="max-width:150mm">montou, esperou com paciência e cuidou de um aquário de verdade, com plantas, camarões e um betta.</p>'
            '<div class="row" style="gap:6mm; margin-top:4mm">' + ''.join(icon('jogo', '18mm') for _ in range(3)) + '</div>'
            '<p style="font:600 18pt/1.2 var(--display); color:var(--ink)">Aquarista de verdade!</p>'
            '<div class="row" style="gap:16mm; margin-top:auto"><div><span class="linha" style="width:60mm"></span><p class="small">data</p></div>'
            '<div><span class="linha" style="width:60mm"></span><p class="small">assinatura do adulto</p></div></div></div>')
...
def contracapa():
    ...
    body = ('<div style="position:absolute; inset:24mm 20mm; display:flex; flex-direction:column; align-items:center; text-align:center; gap:10mm; color:#fff">'
            '<p style="font:600 26pt/32pt var(--display)">Todo grande aquarista começou com um aquário pequeno.</p>'
            f'<div style="width:150mm">{g}</div>'
            '<p style="font:400 14pt/20pt var(--texto); max-width:150mm">Um livro para montar um aquário de 20 litros com plantas, camarões e um betta — e para aprender, esperar, observar e brincar com ele durante o ano todo.</p>'
            '<p style="margin-top:auto; font:600 15pt/1 var(--display)">Vinte litros de mundo · edição para crianças</p></div>')
